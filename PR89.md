...
- extends the runtime seam beyond Hermes-only assumptions
...
This branch no longer only has a single Hermes-shaped runtime path.
It now includes:
...
- reduced Hermes-specific assumptions leaking into Hermes/demo/custom paths
...
- provider selection is no longer effectively Hermes-only
